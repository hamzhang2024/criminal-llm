...
### 3. 整卷完成时落结构化产出 + .done 内容指纹

实现期发现：合并阶段对 .done 卷原是用 `_parse_evidence_blocks` 重解析 evid
展示文本——实测会产出以卷名命名的大杂烩垃圾块（该路径从未真正跑过）。
因此：

- 整卷提取成功时，除 evid 展示文件外，写入 `_evidence_list.json`
  （结构化条目，含 md_file/_temp_dir 引用，与新鲜提取的内存结构一致）；
  合并阶段对 .done 卷**直读该 JSON**，不再二次解析
- .done 文件写入 md 正文字符数（text_len 指纹）；校验不一致则删 .done 重提——
  覆盖"md 重转/修复后内容变了但标记还在"的复活场景
