...
## 9. 实测结论（2026-08-08，真实案卷实测）

实测材料：顾某某诈骗案第 7 卷（两组：① 第 110-117 页银行流水扫描件 8 页；② 第 13/100-102 页手机截图+汇款回单拼贴 4 页）。

| 验收标准 | 结果 |
|----------|------|
| 1. 图片位置出现识别文字 | ✅ 通过。开启后手机截图交易字段（交易时间/付款方/交易类型）、聊天截图文字、印章文字（"自助渠道业务专用章"×4，useSealRecognition 生效）均进入 MD；baseline 均无 |
| 2. 准确率 ≥ 90% | ⚠️ 条件通过。**API 原始输出**：流水表格金额抽样 6/6 正确（9,508/8,051/13,301/25,578/32,494/9,292 逐一与原图比对）。**但发现我方后处理链严重缺陷**：`paddleocr_remote.py:394` 规则 8 `>\d+>{0,3}-?\)?` 会吞掉 `<td>` 后紧跟的数字（`>8,051` → `,051`），最终 MD 中金额全部被破坏。修复后需复验（见计划 Task 3A） |
| 3. `<img>` 标签保留 | ✅ 保留（凭证页样本 10 个标签）→ Task 4 折叠正则修复执行 |
| 4. 耗时增幅 ≤ 50% | ✅ 通过。凭证页 14s vs baseline 16s（4 页）；流水页两模式均约 5 分钟（8 页，排队主导），无显著增幅 |

**实测新发现（已知限制，API 侧、两种模式共有，不阻塞）**：
- 整页聊天截图拼贴页可能整页无输出（4 页样本中第 100 页返回空）
- 拼贴页中的汇款回单关键字段（金额/汇款编号/账号）可能漏识别
- 后续如需提升，可考虑"整页无输出时回退整页单图识别"，当前版本不做
