...
**下游消费**：经核实，4d 综合结论、4.5 控辩对抗、步骤 5 辩护意见目前只读取 `03-证据分析`/`04-法律依据` 等目录，**不会**自动读取 `02-事实要素`。需要在三处的上下文装配代码中显式注入资金流页（各约 3-5 行改动）：4d prompt 增加"资金流梳理"小节与分析项；4.5 的 `context_parts` 增加一段；步骤 5 的 `context` join 增加一段。`00-index.md` 索引生成器已遍历 `02-事实要素`，无需改动。
