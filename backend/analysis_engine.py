...
每处注明涉及的证据编号。"""},
    ...
    def _stage5_sections_dir(self, charge: Optional[str]) -> Path:
        """5C 子章节产物目录（与 stage_5 同层，多罪名走罪名层；断点续跑按节跳过）"""
    ...
    async def stage_5_full_defense(
        ...
        # ----- 5C：三阶层辩护报告（6 个单任务子调用，参照 pipeline 5a-5f 拆分）-----
        if progress_cb:
            progress_cb("正在生成三阶层辩护报告...")

        # 六子调用共享的材料段（逐字节一致，命中 prompt 缓存前缀）：
        # 辩护思路（如有）+ 阶段 1-4/3.5/5A/5B 产物 + 罪名知识 + 三阶层理论 + 构成要件方法
        defense_material = f"""## 辩护对象
被告人：**{defendant}**
...
{CONSTITUTIVE_ELEMENT_ANALYSIS}"""
        ...
        material = strategy_prefix + defense_material

        # 六子调用共享的 system（引用规则等静态要求集中于此，保证前缀一致）
        shared_system = """你是资深刑事辩护律师，正在撰写三阶层综合辩护分析报告（按章节分批输出，本次只写指定章节）。
要求：
...
7. 对指控的每一项事实，都要指出是否有独立证据支撑、证据是否充分（起诉书的指控不等于有证据支撑）""" + _NO_CHITCHAT
        ...
        # 六节指令：材料共享，指令各异（各节保留原有的特有要求，单任务聚焦）
        sub_sections = [
            ("01-案件概述.md", "一、辩护概要",
...
1. **构成要件符合性**：逐项分析指控罪名的构成要件是否符合法条及司法解释的描述，用表格呈现：
...
不要输出章节标题，直接写正文（章节标题由合并时统一添加）。"""),
        ...
        for idx, (filename, heading, instruction) in enumerate(sub_sections, 1):
            ...
            if section_file.exists():
                ...
                if existing.strip():
                    logger.info(f"[阶段5C] {heading} 章节已存在，跳过（断点续跑）")
