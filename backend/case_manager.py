...
- 同一人的第N次笔录，在证据名称中标注次序（如"冯叶飞第三次讯问笔录"）
...
- **详细摘要**：[对讯问/询问笔录，用问答形式保留关键原文摘录（不少于5个问答对）；对书证，列明具体数据；对文书，概括核心内容]
- **原文摘录**：[关键原文的直接引用，不少于3-5段，标注页码或原文位置。对起诉意见书/起诉书，必须摘录每笔犯罪事实的完整原文段落（包括时间、地点、人员、行为、金额），不得概括简化]
...
- **你必须在当前这一次回复中输出本文件的所有证据，严禁分批输出**
- 本系统不支持"后续回复"，你的输出到此为止，没有下一次机会
- **严禁输出以下任何表述（违反视为严重错误）**：
  - "后续还有...将在后续回复中继续"
  - "因篇幅限制，下一轮继续提取"
  - "受篇幅所限，将分批输出"
  - "我将继续在后续消息中完成"
  - "因内容较多，分多次展示"
  - 或任何暗示"分批/延迟/推迟/后续回复/下一轮"的措辞
- **如果你发现自己在考虑"分批输出"，立即停止这种想法，强制在当前回复中输出所有内容**

**完成确认要求**：

- 在所有证据输出完毕后，必须在最后一行添加确认标记：
  `[完成确认] 本文件共提取 N 份证据，全部输出完毕`
- N 必须等于文件中的实际文书数量，如果不符说明你遗漏了，必须补充
