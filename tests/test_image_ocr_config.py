...
def test_default_disabled(tmp_path, monkeypatch):
    """未配置时默认关闭（单图回填太慢，默认不做）"""
...
def test_existing_config_without_key_defaults_false(tmp_path, monkeypatch):
    """老用户升级场景：配置文件存在但无该键，合并 DEFAULTS 后为 False"""
    cfg = tmp_path / "cfg.json"
...
def test_config_status_includes_key(tmp_path, monkeypatch):
    ...
    assert status["image_ocr_enabled"] is False
