...
- llm_model_heavy 为空时 _get_heavy_model() 返回 None（不启用分层，走默认模型）
...
def test_pipeline_final_product_calls_pass_model_override():
    ...
    assert src.count("model_override=_get_heavy_model()") == 7
...
def test_engine_stage_5c_calls_pass_model_override():
    ...
    assert src.count("model_override=_get_heavy_model()") == 1
    # 确认该处在 stage_5_full_defense 的 5C 循环内
    stage5_start = src.index("async def stage_5_full_defense")
    override_pos = src.index("model_override=_get_heavy_model()")
...
def test_heavy_call_sites_share_prefix_structure():
    ...
    assert re.search(
        r"messages = build_cached_messages\(shared_system, material, instruction\)[\s\S]*?section_content = await self\.llm\.chat\(messages, model_override=_get_heavy_model\(\)\)",
        src,
    )
