      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f5f5f7', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '100%', maxWidth: 380, padding: '40px 36px', background: '#fff', borderRadius: 16, textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
            <div style={{ width: 56, height: 56, margin: '0 auto 16px', background: '#34c759', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <button onClick={() => navigate('/login')} style={{ width: '100%', padding: '12px', background: '#007aff', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 15, fontWeight: 600 }}>返回登录</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f5f5f7', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '12px 20px', borderBottom: '1px solid var(--macos-border)', background: '#fafafa', gap: '12px' }}>
        <button onClick={() => navigate('/login')} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', color: '#007aff', borderRadius: '6px' }}
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: '100%', maxWidth: 380, padding: '40px 36px', background: '#fff', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{ width: 56, height: 56, margin: '0 auto 12px', background: '#007aff', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'flex', marginBottom: 24, background: '#f5f5f7', borderRadius: 8, padding: 3 }}>
            {[
              ...
              <button key={tab.key} onClick={() => setMode(tab.key)} style={{
                ...
                color: mode === tab.key ? '#007aff' : '#86868b', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          ...
          {mode === 'email' && (
            <form onSubmit={handleResetEmail}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" onClick={handleSendCode} disabled={loading || countdown > 0} style={{
                    padding: '0 16px', background: countdown > 0 ? '#f5f5f7' : '#007aff', color: countdown > 0 ? '#86868b' : '#fff',
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '12px', background: loading ? '#86868b' : '#007aff', color: '#fff', border: 'none', borderRadius: 8, cursor: loading ? 'not-allowed' : 'pointer', fontSize: 15, fontWeight: 600 }}>
                {loading ? '重置中...' : '重置密码'}
              </button>
            </form>
          )}
